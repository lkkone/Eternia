...
if (isset($_SESSION['loginadmin']) && $_SESSION['loginadmin'] <> '') {
    ...
    $stmt = $connect->prepare("UPDATE about SET title = ?, aboutimg = ?, info1 = ?, info2 = ?, info3 = ?, btn1 = ?, btn2 = ?, infox1 = ?, infox2 = ?, infox3 = ?, infox4 = ?, infox5 = ?, infox6 = ?, btnx2 = ?, infof1 = ?, infof2 = ?, infof3 = ?, infof4 = ?, btnf3 = ?, infod1 = ?, infod2 = ?, infod3 = ?, infod4 = ?, infod5 = ? WHERE id = ?");
    if ($stmt) {
        $stmt->bind_param("ssssssssssssssssssssssssi", $title, $aboutimg, $info1, $info2, $info3, $btn1, $btn2, $infox1, $infox2, $infox3, $infox4, $infox5, $infox6, $btnx2, $infof1, $infof2, $infof3, $infof4, $btnf3, $infod1, $infod2, $infod3, $infod4, $infod5, $id);
        $result = $stmt->execute();
        if (!$result) {
            error_log("aboutpost.php SQL执行失败: " . $stmt->error);
        }
        $stmt->close();
    if ($result) {
        echo "1";
        ...
        error_log("aboutpost.php SQL准备失败: " . $connect->error);
        echo "0";
    }
} else {
